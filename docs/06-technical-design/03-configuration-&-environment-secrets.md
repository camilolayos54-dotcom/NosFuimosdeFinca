...
# SENSIBLE â€” Llave de acceso total, salta politicas Spring Security + Row-Level Filtering (Row Level Security).
...
# SENSIBLE â€” Secreto criptografico para validar el Hash de los Webhooks (Previene inyeccion de pagos falsos).
...
- **Phase 7 â€” D2 (Project Scaffolding):** El desarrollador tomara el contenido de estos dos bloques, quitara los comentarios `[PROJECT_ID]` / `eyJhb...` y creara un archivo `.env.example` literal en el repositorio de codigo, exigiendo a todos llenar sus valores locales.
- **Phase 9 â€” D3 (Infrastructure as Code / SecOps):** Operaciones utilizara la lista del apartado 2 (Backend) para crear los Secretos cifrados en el entorno Cloud de produccion antes del lanzamiento (Go-Live).
